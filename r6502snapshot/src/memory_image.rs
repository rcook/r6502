...
impl MemoryImage {
    ...
    #[must_use]
    pub fn slice(&self, range: &AddressRange) -> MemorySlice {
        let memory_start = self.load().unwrap_or_default() as usize;
        let bytes = self.bytes();
        let memory_end = memory_start + bytes.len();
        let range_start = range.start() as usize;
        let range_end = range.end() as usize + 1;

        if range_end <= memory_start || range_start >= memory_end {
            return MemorySlice {
                bytes: &[],
                load: 0,
            };
        ...
        let effective_start = range_start.max(memory_start);
        let effective_end = range_end.min(memory_end);

        let bytes_start = effective_start - memory_start;
        let bytes_end = effective_end - memory_start;

        let load = if range_start < memory_start {
            u16::try_from(memory_start - range_start).unwrap()
        } else {
            0
        };
    ...
    fn from_reader<R: Read + Seek>(mut reader: R) -> Result<Self> {
        if let Some(image) = Image::try_from_reader(&mut reader)? {
...
";
    ...
    fn slice() {
        ...
        let memory_image = MemoryImage::Other(OtherImage::new_sim6502(
            0x0005,
            0x0000,
            0xff,
            (1..=16).collect(),
