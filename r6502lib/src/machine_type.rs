    ...
    pub(crate) fn get_device_descriptions(self) -> Vec<DeviceDescription> {
        match self {
            Self::AllRam => vec![DeviceDescription {
                ...
                device_fn: Box::new(|_, _, _| Box::new(Ram::<MEMORY_SIZE>::default())),
                offset: 0x0000,
            }],
            Self::Custom => vec![
                DeviceDescription {
                    address_range: AddressRange::new(0x0000, 0xfbff).expect("Must succeed"),
                    device_fn: Box::new(|_, _, _| Box::new(Ram::<{ 0xfbff + 1 }>::default())),
                    offset: 0x0000,
                },
                DeviceDescription {
                    address_range: AddressRange::new(0xfc00, 0xfc03).expect("Must succeed"),
                    device_fn: Box::new(|bus_tx, _, _| Box::new(Pia::new(bus_tx))),
                    offset: 0xfc00,
                },
                DeviceDescription {
                    address_range: AddressRange::new(0xfc04, 0xffff).expect("Must succeed"),
                    device_fn: Box::new(|_, _, _| {
                        Box::new(Ram::<{ 0xffff - 0xfc04 + 1 }>::default())
                    }),
                    offset: 0xfc04,
                },
            ],
            Self::Acorn => vec![
                DeviceDescription {
                    ...
                    device_fn: Box::new(|_, _, _| Box::new(Ram::<0x8000>::default())),
                    offset: 0x0000,
                },
                DeviceDescription {
                    address_range: AddressRange::new(0x8000, 0xffff).expect("Must succeed"),
                    device_fn: Box::new(|_, _, _| Box::new(Rom::<0x8000>::default())),
                    offset: 0x8000,
                },
            ],
            Self::Apple1 => vec![
                DeviceDescription {
                    address_range: AddressRange::new(0x0000, PIA_START_ADDR - 1)
                        .expect("Must succeed"),
                    device_fn: Box::new(|_, _, _| {
                        Box::new(Ram::<{ PIA_START_ADDR as usize }>::default())
                    }),
                    offset: 0x0000,
                },
                DeviceDescription {
