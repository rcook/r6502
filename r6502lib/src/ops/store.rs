...
pub(crate) fn dec(s: &mut VmState, addr: u16) {
    let result = s.memory.load(addr).wrapping_sub(1);
    s.memory.store(addr, result);
...
pub(crate) fn inc(s: &mut VmState, addr: u16) {
    let result = s.memory.load(addr).wrapping_add(1);
    s.memory.store(addr, result);
    set_flags_on_value(s, result);
}
