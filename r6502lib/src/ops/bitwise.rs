...
pub(crate) fn and(state: &mut Cpu, operand: u8) {
    state.reg.a &= operand;
...
pub(crate) fn bit(state: &mut Cpu, operand: u8) {
...
pub(crate) fn eor(state: &mut Cpu, operand: u8) {
    state.reg.a ^= operand;
    set_flags_on_value(state, state.reg.a);
}
...
pub(crate) fn ora(state: &mut Cpu, operand: u8) {
    state.reg.a |= operand;
    set_flags_on_value(state, state.reg.a);
}
